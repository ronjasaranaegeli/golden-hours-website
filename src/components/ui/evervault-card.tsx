  ...
  return (
    ...
    >
      <div
        ...
        className="group/card rounded-none w-full relative overflow-hidden bg-[#F9F6F0] flex items-center justify-center h-full"
      >
        <CardPattern
          mouseX={mouseX}
          mouseY={mouseY}
        ...
        <div className="relative z-10 flex items-center justify-center">
          <div className="relative px-6 py-3 md:px-8 md:py-4 rounded-full flex items-center justify-center text-[#2D2A24] font-bold">
            <div className="absolute w-full h-full bg-[#F9F6F0]/[0.85] blur-sm rounded-full" />
            <span className="text-[#2D2A24] z-20 font-serif">{text}</span>
          </div>
        </div>
      </div>
    </div>
...
export function CardPattern({ mouseX, mouseY, randomString, isTouch }: any) {
  ...
  return (
    <div className="pointer-events-none">
      ...
      <motion.div
        className="absolute inset-0 bg-gradient-to-r from-[#F9F6F0] to-[#F3F0EA] opacity-100 backdrop-blur-xl transition duration-500"
        style={style}
      />
      <motion.div
        ...
        style={style}
      >
        <p className="absolute inset-x-0 inset-y-0 h-full w-full break-words whitespace-pre-wrap text-[#2D2A24] font-['Courier_New'] transition duration-500 overflow-hidden text-xl md:text-2xl lg:text-3xl">
          {randomString}
        </p>
      </motion.div>
    </div>
